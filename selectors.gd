...
@onready var spin_box: SpinBox = $ViewsSpinBox
@onready var slot_dropdown: OptionButton = $ViewOptionButton
@onready var file_dialog: FileDialog = $ViewOptionButton/FileDialog

# Dataset information (shared for all slots)
var dataset_dir: String

var detections_dirs: Array[String] = []          # one path per slot
var current_detection_slot: int = -1             # which slot is selected in the dropdown

# Per-slot caches (index == slot index)
var slot_detections_category_maps: Array = []    # Array[Dictionary image_id -> {seg_id: cat_name}]
var slot_matching_maps: Array = []               # Array[Dictionary image_id -> matching]
...
func _on_spinbox_value_changed(value: float) -> void:
	...
	slot_detections_category_maps.resize(count)
	slot_matching_maps.resize(count)

	# Ensure each slot has Dictionaries
	for i in range(count):
		if typeof(slot_detections_category_maps[i]) != TYPE_DICTIONARY:
			slot_detections_category_maps[i] = {}
		if typeof(slot_matching_maps[i]) != TYPE_DICTIONARY:
			slot_matching_maps[i] = {}

	# Rebuild the dropdown items
	slot_dropdown.clear()
	for i in count:
		slot_dropdown.add_item("View %d" % (i + 1))

	if count > 0:
		slot_dropdown.select(0)
		current_detection_slot = 0
	else:
		current_detection_slot = -1

	_update_visible_image_ids()


func _on_slot_dropdown_item_selected(index: int) -> void:
	current_detection_slot = index

	# If this slot has no folder yet, open the dialog to pick one
	if index < 0 or index >= detections_dirs.size() or detections_dirs[index] == "":
		file_dialog.popup_centered()
	else:
		# Folder already set -> just recompute which images are visible
		_update_visible_image_ids()


# Helper: load detections.json for a specific slot and cache it
func _load_detections_for_slot(slot: int, dir: String) -> void:
	if slot < 0:
		return

	print("Detections folder for slot %d: %s" % [slot, dir])
	...
	if local_det_maps.is_empty():
		push_warning("No detection entries with image_name found in detections.json for slot %d." % slot)

	# Cache into per-slot arrays
	slot_detections_category_maps[slot] = local_det_maps
	slot_matching_maps[slot] = local_match_maps
	detections_dirs[slot] = dir


# CALLED FROM YOUR DATASET FOLDER FILE DIALOG
...
# CALLED FROM THE DETECTIONS FOLDER FILE DIALOG
func _detections_folder_selected(dir: String) -> void:
	if current_detection_slot < 0:
		push_error("No detection slot selected. Select a slot in the dropdown first.")
		return

	# Load and cache detections for this slot
	_load_detections_for_slot(current_detection_slot, dir)

	# Recompute the visible image list (intersection GT ∩ detections of current slot)
	_update_visible_image_ids()
...
func _update_visible_image_ids() -> void:
	...
	# Use detections from the *current* slot (for filtering only)
	var det_maps: Dictionary = {}
	if current_detection_slot >= 0 and current_detection_slot < slot_detections_category_maps.size():
		det_maps = slot_detections_category_maps[current_detection_slot]

	if det_maps.is_empty():
		# No detections loaded for current slot: use all GT ids
		image_ids = gt_image_ids.duplicate() as Array[String]
	else:
		# Detections loaded: only keep images that exist in both
		for id in gt_image_ids:
			if det_maps.has(id):
				image_ids.append(id)

		if image_ids.is_empty():
			push_warning("No common images between ground truths and detections for current view.")
...
func _load_image(image_id: String) -> void:
	...
	for slot in range(detections_dirs.size()):
		...
		if slot < slot_detections_category_maps.size():
			slot_det_maps = slot_detections_category_maps[slot]
		if slot < slot_matching_maps.size():
			slot_match_maps = slot_matching_maps[slot]
